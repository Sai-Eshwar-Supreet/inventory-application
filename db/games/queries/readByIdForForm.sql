WITH genres_data AS (
    SELECT gg.game_id, ARRAY_AGG(ge.id) AS genres
...
developers_data AS(
    SELECT gd.game_id, ARRAY_AGG(d.id) AS developers
...
SELECT 
    ...
    COALESCE(genres_data.genres, ARRAY[]::int[]) AS genres,
    COALESCE(developers_data.developers, ARRAY[]::int[]) AS developers
FROM games AS g
